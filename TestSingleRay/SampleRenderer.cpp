...
namespace osc {
	...
	void osc::TriangleMesh::loadOFF(std::string& filename)
	{
		...
		std::string line;
		file >> line;
		if (line == "noff" || line == "NOFF") {
			hasNormals = true;
		}
		else if (line != "OFF") {
			std::cerr << "无效的 OFF 文件: " << filename << std::endl;
			file.close();
			return;
		}
		...
		for (int i = 0; i < numFaces; ++i) {
			int vertexCount;
			file >> vertexCount; // OFF 面的第一个数字表示该面的顶点数，通常是 3（三角形）
			if (vertexCount != 3) {
				std::cerr << "非三角形面，当前只支持三角形。" << std::endl;
			}
	...
	void SampleRenderer::render()
	{
		...
		{
			int32_t nPoints = min(batchPoints, totalPoints - offset);
	...
	void SampleRenderer::resize(const vec2i& newSize)
	{
		...
		hitIDBuffer.resize(newSize.x * newSize.y * sizeof(uint32_t));
		...
		launchParams.hitTool.size = newSize;
		launchParams.hitTool.hitID = (uint32_t*)hitIDBuffer.d_pointer();
	...
	void SampleRenderer::downloadHitID(uint32_t h_hitID[])
	{
		hitIDBuffer.download(h_hitID,
